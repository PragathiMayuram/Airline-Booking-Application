window.renderSeatSelection = function () {
    ...
    for (let i = 0; i < numPassengers; i++) {
      ...
      label.textContent = `Seat ${i + 1} Selection:`;
      const select = document.createElement("select");
      select.className = "seat-select";
      select.innerHTML = `
        <option value="Middle">Middle - $100</option>
        <option value="Aisle">Aisle - $130</option>
        <option value="Window">Window - $150</option>
      `;
  ...
  window.updateTotalPrice = function () {
    ...
    selects.forEach(select => {
      ...
      else if (seatType === "Aisle") total += 130;
      else total += 100;
    });
